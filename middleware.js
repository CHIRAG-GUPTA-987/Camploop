...
const {campgroundSchema, reviewSchema} = require('./Validations/validateschemas')
...
const isAuthor = async(req, res, next)=>{
    ...
    req.flash('error', `You don't have permission to modify or delete ${camp.title}`)
...
const isReviewAuthor = async(req, res, next)=>{
    ...
    if(review.author.equals(req.user._id) || camp.author.equals(req.user._id))return next();
    req.flash('error', `You don't have permission to modify or delete review made by ${review.author.username}`)
    res.redirect(`/campground/${id}`)
}
